 ...
 * @since 3.2
 */
public class PDBSCANClusterer<T extends Clusterable> extends Clusterer<T> {
     ...
     */
    public PDBSCANClusterer(final double eps, final int minPts)
     ...
     * @throws NotPositiveException if {@code eps < 0.0} or {@code minPts < 0}
     */
    public PDBSCANClusterer(final double eps, final int minPts, final DistanceMeasure measure)
        throws NotPositiveException {
        super(measure);
    ...
    public List<Cluster<T>> cluster(final Collection<T> points) throws NullArgumentException {
        ...
        distances=ArrayTable.create(points, points);
        ...
        for (final T point : points) {
            ...
            i++;
            if(i%10==0)
            	System.out.println("\tClustered "+i+"/"+points.size());
        }
    ...
    private Table<Clusterable, Clusterable, Double> distances;
    
    @Override
    protected double distance(Clusterable p1, Clusterable p2) {
    	Double d=distances.get(p1, p2);
    	if(d==null) {
    		d=super.distance(p1, p2);
    		distances.put(p1, p2, d);
    		distances.put(p2, p1, d);
    	}
    	return d;
    }
