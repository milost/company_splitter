...
public class IGCalculatorAbs {
	...
	public static void main(String[] args) throws JsonSyntaxException, IOException {
		...
		for(SimpleFeature f:FeatureManager.ALL.getSimpleFeatures()) {
			...
			for(int window:WINDOW_SIZES) {
				...
				double entropy=entropy2(tokens, f, TAG_FUNCTION, window);
			...
			for(int window:WINDOW_SIZES) {
				...
				double entropyS=entropy2(tokens, f, S_TAG_FUNCTION, window);
		...
		for(SimpleFeature f1:FeatureManager.ALL.getSimpleFeatures()) {
			for(SimpleFeature f2:FeatureManager.ALL.getSimpleFeatures()) {
				if(f1!=f2) {
					...
					for(int window:WINDOW_SIZES) {
						double generalEnt=entropy2(tokens, f1, TAG_FUNCTION, window);
						double entropy=entropy3(tokens, f1, f2, TAG_FUNCTION, window);
					...
					for(int window:WINDOW_SIZES) {
						double generalEntS=entropy2(tokens, f1, S_TAG_FUNCTION, window);
						double entropyS=entropy3(tokens, f1, f2, S_TAG_FUNCTION, window);
		...
		for(IComplexFeature f:FeatureManager.ALL.getComplexFeatures()) {
			...
			double best=0;
				double generalEnt=entropy1(tokens, TAG_FUNCTION);
				double entropy=entropy2(tokens, f, TAG_FUNCTION);
				System.out.println("  IG :\t "+NUMBER.format(generalEnt-entropy));
				best=Math.max(best, generalEnt-entropy);
				double generalEntS=entropy1(tokens, S_TAG_FUNCTION);
				double entropyS=entropy2(tokens, f, S_TAG_FUNCTION);
	...
	public static double entropy1(Collection<Token> tokens, TagExtractor extractor) {
	...
	public static double entropy2(Collection<Token> tokens, IComplexFeature f, TagExtractor extractor) {
		ListMultimap<String,Token> counter = ArrayListMultimap.create();
		for(Token t:tokens) {
