...
public class Suffix extends StringFeature {
	private int length;
	
	/** only for serialization **/
	public Suffix() {this(-1);}
	...
	public void calculateFeatures(Token[] tokens) {
		for(Token t:tokens) {
			t.setFeature(this, t.getRawForm().substring(Math.max(0, t.getRawForm().length()-length), t.getRawForm().length()));
		}
	}
